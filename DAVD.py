...
# ================= CẤU HÌNH TRANG (BẮT BUỘC ĐẶT ĐẦU TIÊN) =================
st.set_page_config(page_title="Minh Quest 90", page_icon="🚀", layout="wide")

# ================= SỬA LỖI 1: CỐ ĐỊNH ĐƯỜNG DẪN TUYỆT ĐỐI =================
# Giúp file CSV luôn luôn nằm chung thư mục với file DAVD.py, không bao giờ lo mất dữ liệu
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
CSV_FILE = os.path.join(BASE_DIR, "nhat_ky_diem_xp.csv")
...
def save_data(date, morning_xp, day_xp, evening_xp, tasks_done):
    ...
    # Ép kiểu cột Ngày về chuỗi để so sánh chính xác không lo lệch định dạng
    df["Ngày"] = df["Ngày"].astype(str)

    # Cập nhật nếu đã có, ngược lại thêm mới
    if date_str in df["Ngày"].values:
        df.loc[df["Ngày"] == date_str, "Morning_XP"] = morning_xp
        df.loc[df["Ngày"] == date_str, "Day_XP"] = day_xp
        df.loc[df["Ngày"] == date_str, "Evening_XP"] = evening_xp
        df.loc[df["Ngày"] == date_str, "Tổng_XP_Ngày"] = total_xp
        df.loc[df["Ngày"] == date_str, "Chi_Tiet_Nhiem_Vu"] = tasks_str
    else:
...
def get_current_week_xp():
    ...
    for index, row in df.iterrows():
        try:
            # Ép kiểu chuỗi ngày và cắt bỏ phần giờ (nếu có)
            ngay_str = str(row["Ngày"]).split(" ")[0]
            row_date = datetime.datetime.strptime(ngay_str, "%Y-%m-%d").date()
            year, week, _ = row_date.isocalendar()
            if year == current_year and week == current_week:
                total_week_xp += float(row["Tổng_XP_Ngày"])
        except (ValueError, TypeError, KeyError):
            pass
    return int(total_week_xp)
...
def main():
    ...
    with tab1:
        ...
        # Đảm bảo so khớp chuỗi chuẩn xác
        df["Ngày"] = df["Ngày"].astype(str)
        ...
        # ================= SỬA LỖI 2: THÊM KEY RIÊNG CHO TỪNG Ô THEO NGÀY =================
        with col1:
